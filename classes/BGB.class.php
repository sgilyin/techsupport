...
*/
        $bgb_result = $mysqli->query("SELECT tbl_cpt1_abonent.val abonent, CONCAT(tbl_street.title, ' д. ', tbl_house.house, CONCAT_WS( ' кв. ',tbl_house.frac, IF(tbl_flat.flat='',NULL,tbl_flat.flat))) address, tbl_cpt1_phone.val phone, tbl_cpt1_comment.val comment, IF(tbl_contract.status=0,'Активен','Не активен') status, tbl_cb.summa1+tbl_cb.summa2-tbl_cb.summa3-tbl_cb.summa4 balance, GROUP_CONCAT(tbl_tariff.tariff) tariff, tbl_id15.host host, tbl_idps15.port port, tbl_ist15.title type
FROM contract tbl_contract
LEFT JOIN contract_parameter_type_1 tbl_cpt1_abonent ON (tbl_contract.id=tbl_cpt1_abonent.cid AND (tbl_cpt1_abonent.pid=1 OR tbl_cpt1_abonent.pid=6))
...
GROUP BY tbl_contract.id")->fetch_object();
